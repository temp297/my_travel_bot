...
try:
    ...
    ASSISTANT_MINUTE = int(os.getenv("ASSISTANT_MINUTE", "10"))
except ValueError:
...
async def generate_and_send_ai_tour_post():
    ...
    cta_text = (
        ...
        f"🎁 Приємний бонус: кожен наш клієнт може отримати персональну знижку за програмую лояльності!\n"
    ...
    categories = [
        {"name": "Туреччина", "prompt_part": "Вибери до 5 найкращих готелів 4★ та 5★ СУТО в ТУРЕЧЧИНІ."},
        {"name": "Єгипет", "prompt_part": "Вибери до 5 найкращих готелів 4★ та 5★ СУТО в ЄГИПТІ."},
        {"name": "Україна", "prompt_part": "Вибери до 5 найкращих пропозицій або готелів СУТО в УКРАЇНІ."},
        {"name": "Інші Країни", "prompt_part": "Вибери до 5 найкращих готелів 4★ та 5★ з БУДЬ-ЯКИХ ІНШИХ КРАЇН, окрім Туреччини, Єгипту та України."}
    ]

    for cat in categories:
        prompt = (
            f"Ти — професійний тревел-копірайтер. На основі наступного сирого тексту з туристичними даними склади один цікавий, "
            f"структурований і залучаючий пост для Telegram-каналу українською мовою.\n\n"
            f"ТВОЄ ЗАВДАННЯ: {cat['prompt_part']}\n\n"
            f"Суворо дотримуйся наступних правил та структури оформлення:\n"
            f"1. НІКОЛИ не згадуй назву сторонніх сайтів, звідки взято інформацію.\n"
            f"2. Зроби красивий головний заголовок для цього поста (наприклад: ☀️ <b>ТОП пропозицій: {cat['name']}</b>).\n"
            f"3. Для КОЖНОГО готелю у вибірці суворо використовуй наступний візуальний шаблон (слово в слово, з новими рядками та емодзі):\n\n"
            f"📍 <b>КРАЇНА (Регіон/Курорт)</b>\n"
            f"🏨 <b>Назва готелю і зірковість (наприклад: Hilton Marsa Alam 5*)</b>\n"
            f"🍽 <b>Харчування:</b> [Вкажи тип, наприклад: All Inclusive / Сніданки]\n"
            f"📅 <b>Виліт/Дата:</b> [Вкажи дату та кількість ночей, наприклад: 31.05 (6 ночей)]\n"
            f"💰 <b>Ціна:</b> [Вкажи ціну у валюті з тексту, наприклад: 886 €]\n"
            f"[Тут напиши художній, захоплюючий опис готелю та його території на 2-3 речення]\n"
            f"➕ <b>Плюси:</b> [На основі відгуків коротко вкажи 1-2 головні переваги готелю]\n"
            f"➖ <b>Мінуси:</b> [Коротко вкажи 1 незначний мінус або нюанс, щоб текст виглядав чесно й об'єктивно]\n\n"
            f"5. ЛІМІТ СИМВОЛІВ: Оскільки опис детальний, пиши ємно. Твій згенерований текст разом із заголовком ОБОВ'ЯЗКОВО має бути коротшим за 3400 символів, щоб разом із рекламою поміститися в ліміт Telegram (4096). Використовуй тільки HTML-теги <b> та <i>.\n\n"
