...
async def fetch_tat_ua_data():
    ...
    logging.info("🚀 [ПАРСЕР] Початок ОДНОРАЗОВОГО швидкого збору даних з сортуванням зірковості через HTTPX...")
...
async def generate_and_send_travel_news():
    ...
    # --- 2. ЗЧИТУВАННЯ МИНУЛИХ НОВИН З БД ДЛЯ БЛОКУВАННЯ ПОВТОРІВ ---
    ...
    try:
        ...
        logging.error(f"⚠️ Помилка зчитування історії новин з БД: {db_read_err}")

    # --- 3. ДИНАМІЧНИЙ ФОКУС РЕГІОНУ (РОТАЦІЯ ДЛЯ СВІТОВИХ НОВИН) ---
    main_focuses = [
        "Додатково приділи увагу Азії (Таїланд, Балі, ОАЕ) та авіаперельотам з сусідніх країн ЄС.",
        "Додатково приділи увагу Балканам (Туреччина, Болгарія, Чорногорія) та курортним напрямкам.",
        "Додатково приділи увагу Європі, новим правилам перетину кордону в ЄС (EES/ETIAS) та аеропортам Молдови/Румунії.",
        "Додатково приділи увагу Африці/Екзотиці (Єгипет, Занзібар, Мальдіви) та візовим спрощенням (eVisa)."
    ]
    ...
    # Словник для точної назви дня тижня українською
    weekdays_ua = {
        0: "понеділок",
        1: "вівторок",
        2: "середа",
    ...
    today_weekday = weekdays_ua[now.weekday()]
    ...
    prompt = (
        ...
        f"Суворо заборонено плутати дні тижня (наприклад, називати {today_weekday} 'п'ятницею' чи 'понеділком').\n\n"
        f"🔍 КРИТИЧНА ВИМОГА ДЛЯ ПОШУКУ В ІНТЕРНЕТІ (GROUNDING):\n"
        f"- ПЕРЕД генерацією тексту обов'язково виконай реальний пошук в Google за останні 24–48 годин!\n"
        f"- Знайди СВІЖІ та ДІЙСНІ новини за запитами: 'ДПСУ стан пунктів пропуску черги', 'Lowcost.ua новини', 'Avianews', 'новини туризму візи авіація'.\n"
        f"- Твій пост має базуватися ТІЛЬКИ на реальних фактах, які ти знайшов у пошуку за останні дні.\n\n"
        f"Напиши свіжий, яскравий, практичний, унікальний та інформативний дайджест світових туристичних новин "
        f"та актуальної логістики для українців.\n\n"
        f"🚨 ОБОВ'ЯЗКОВИЙ ЩОДЕННИЙ БЛОК ПРО КОРДОН (ПОВИНЕН БУТИ У КОЖНОМУ ПОСТІ):\n"
        f"- Пункт 1 дайджесту ЗАВЖДИ має стосуватися ситуації на українському кордоні (Західний кордон, Польща, Молдова, Румунія, Угорщина, Словаччина), "
        f"наявності черг, затримок, ремонтних робіт, пошкоджень інфраструктури або діючих зручних маршрутів.\n"
        f"- ⚠️ ПРАВИЛО УНІКАЛЬНОСТІ ДЛЯ КОРДОНУ: Якщо якась конкретна подія чи факт про КПП вже висвітлювалися в історії нижче (наприклад, про ремонт чи закриття конкретного пункту), НЕ ПОВТОРЮЙ цю саму новину! Замість цього зроби акцент на ІНШОМУ пункті пропуску (наприклад, перейди з Польщі на Румунію/Молдову), або подай загальне актуальне зведення про поточний режим роботи та черги на сьогодні.\n\n"
        f"🎯 ДОДАТКОВИЙ ДИНАМІЧНИЙ НАПРЯМОК ДНЯ (ДЛЯ 2-4 ПУНКТІВ НОВИН):\n"
        f"{today_focus}\n\n"
        f"⛔ КАТЕГОРИЧНО ЗАБОРОНЕНО СМИСЛОВІ ПОВТОРИ ТА СТАРІ ФАКТИ:\n"
        f"Ось короткий зміст новин, які ВЖЕ ОПУБЛІКОВАНІ в каналі за останні дні:\n"
        f"{past_news_context}\n\n"
        f"⚠️ СУВОРЕ ПРАВИЛО: КАТЕГОРИЧНО ЗАБОРОНЕНО повторювати будь-які світові новини, факти про візи чи конкретні події на кордоні з цього списку! "
        f"Якщо новина вже була висвітлена — ІГНОРУЙ ЇЇ та знайди СВІЖУ та УНІКАЛЬНУ тему.\n\n"
        f"🌐 ДЖЕРЕЛА ТА ОРІЄНТИРИ ДЛЯ ІНФОРМАЦІЇ:\n"
        f"- Використовуй інформацію з БУДЬ-ЯКИХ надійних світових та українських ЗМІ, Держприкордонслужби України (ДПСУ), Lowcost.ua, Avianews, Euronews Travel, Simple Flying, Skift, МЗС України тощо.\n"
        f"- Усі факти про візи та правила в'їзду повинні відповідати дійсним умовам для власників українського біометричного паспорта.\n\n"
        f"⛔ ЗАБОРОНА НА ВИГАДУВАННЯ ТА ГАЛЮЦИНАЦІЇ:\n"
        f"- Категорично ЗАБОРОНЕНО вигадувати новини, факти, дати, авіарейси чи стан кордонів. Усі події мають бути реальними фактами із знайдених новинних джерел.\n"
        f"- Якщо за останні дні немає новин про конкретні нові авіарейси чи візи — НЕ ВИГАДУЙ ЇХ, а напиши про реальні діючі правила або інші актуальні новини туризму.\n\n"
        f"🚫 КАТЕГОРИЧНА ЗАБОРОНА НА САМОРЕКЛАМУ ТА СЛОВО «ФОКУС»:\n"
        f"- Заборонено згадувати електронних помічників, ботів, закликати звертатися до компанії чи писати про консультації.\n"
        f"- КАТЕГОРИЧНО ЗАБОРОНЕНО використовувати слово «Фокус» у заголовках та тексті поста! Пиши природно, як журналіст ЗМІ.\n\n"
        f"🌎 СТРУКТУРА ТА ЗМІСТ ПОСТА:\n\n"
        f"1. <b>Заголовок:</b> яскравий, з прапорами або тематичними емодзі (🌍, ✈️, 🛂, 🚗, 🏝️).\n\n"
        f"2. <b>Привітання та вступ (1-2 речення):</b> Почни пост з короткого дружнього привітання мандрівників (наприклад: 'Привіт, мандрівники! Ми підготували для вас свіжий дайджест...').\n\n"
        f"3. <b>Дайджест новин (2–4 пункти без проміжних підзаголовків):</b> Одразу після привітання з нового рядка виводь нумерований список новин (БЕЗ фраз 'Дайджест світових новин:' чи подібних підзаголовків):\n"
        f"   - <b>Пункт 1 (ОБОВ'ЯЗКОВИЙ): Ситуація на кордоні та логістика:</b> УНІКАЛЬНИЙ актуальний статус КПП (Польща, Румунія, Молдова тощо), черги, ремонт чи транзитні маршрути, про які ще не писали за останні дні.\n"
        f"   - <b>Пункти 2–4: Світові новини та регіональний напрямок:</b> свіжі новини туризму, авіації, віз та відпочинку відповідно до додаткового напрямку дня.\n\n"
        f"4. <b>Важливо для українців / Правила в'їзду:</b> (НЕ використовуй слово 'Фокус'! Вживай фрази 'Важливо для мандрівників' або 'Правила в'їзду') — чітко зазнач УНІКАЛЬНІ умови або юридичні нюанси для перетину кордону, які ще не описувалися у попередніх постах.\n\n"
        f"5. <b>Лайфхак / Практична порада дня:</b> 1-2 речення короткої, СВІЖОЇ та УНІКАЛЬНОЇ поради (щодня міняй тему: онлайн-камери ДПСУ, бронь черг 'єЧерга', страхування, перевезення готівки/тварин, правила багажу лоукостерів тощо — НЕ повторюй лайфхаки з минулих днів).\n\n"
        f"6. <b>Хештеги:</b> 3–5 тематичних хештегів (#подорожісвітом #новинитуризму #кордон #безвіз #подорожі2026).\n\n"
        f"⚠️ СУВОРІ ВИМОГИ ДО ФОРМАТУВАННЯ, ВІДСТУПІВ ТА СТИЛЮ:\n"
        f"- <b>ОБОВ'ЯЗКОВІ ВІДСТУПИ:</b> Відокремлюй КОЖЕН номер новини (1, 2, 3...), розділи пости та блок хештегів ПОРОЖНІМ РЯДКОМ (подвійним `\\n\\n`). Текст ні в якому разі не повинен зливатися в один монолітний абзац!\n"
        f"- Мова: українська.\n"
        ...
        f"- КАТЕГОРИЧНО ЗАБОРОНЕНО використовувати HTML-теги <p>, </p>, <br>, <div>. Для розділення думок використовуй звичайний Enter.\n"
    ...
    try:
        ...
        # 🧹 Очищення від заборонених тегів та нормалізація відступів (стискаємо все > 2 рядків до 1 порожнього рядка)
        news_text = (
            news_text
            .replace("<br>", "\n")
            .replace("<br/>", "\n")
        ...
        full_message = f"{prefix}{header_text}{news_text}"

        # --- 5. НАДСИЛАННЯ ПОВІДОМЛЕННЯ В ТЕЛЕГРАМ ---
        ...
        try:
            news_summary = news_text[:250].replace("\n", " ").strip()
...
# Базовий шаблон для всіх фото з Unsplash (щоб не дублювати його в коді 75 разів)
    UNSPLASH_BASE_URL = "https://images.unsplash.com/{}?fm=jpg&fit=crop&w=1000&h=562&q=75"
    ...
    for index, cat in enumerate(categories):
        ...
        prompt = (
            ...
            f"Ти повинен робити вибірку готелів (до 5 штук) суворо за таким каскадним пріоритетом:\n"
            ...
            f"- Вибирай варіанти готелів за ЯКІСТЮ та НАЙВИГІДНІШОЮ ціною для конкретної кількості ночей.\n"
            f"- Ціна, тип харчування та кількість ночей у шаблоні мають бути СУВОРО СИНХРОНІЗОВАНІ між собою для обраного варіанту.\n"
            ...
            f"- Уникай 'оверпрайсу': не вибирай готелі з космічними цінами, якщо в тексті є чудові варіанти за адекватною ціною. Шукай 'золоту середину' ціни та сервісу.\n\n"
        
            f"⚠️ СУВОРЕ ПРАВИЛО ДЛЯ АНАЛІЗУ ТА ФОРМАТУ ДАТ (ТОЧНИЙ ПОВТОР З САЙТУ):\n"
            f"1. Знайди в тексті конкретного туру дату (день, місяць, рік), яка прописана поруч із обраною ціною. Заборонено вигадувати дані самостійно.\n"
            ...
            f"- Заборонені будь-які технічні чи робочі фрази типу 'Згідно з наявними даними...', 'Ми знайшли...'. Текст повинен виглядати як рекомендація живого експерта.\n"
            f"4. СУВОРЕ ПРАВИЛО ДЛЯ РОЗДІЛЕННЯ ТУРІВ: Відокремлюй картки готелів одну від одної СУВОРО одним порожнім рядком. Категорично ЗАБОРОНЕНО самостійно малювати штучні лінії, ставити знаки мінусів, дефісів чи символи на кшталт '---' між готелями! Тільки порожній рядок.\n"
            ...
            f"📅 <b>Дата заїзду:</b> [Вкажи точну дату туру з тексту СУВОРО в оригінальному форматі сайту ДД.ММ.РРРР або ДД місяця], [Вкажи кількість ночей саме для цієї ціни]\n"
            f"💰 <b>Ціна:</b> [Вкажи саме вартість для обраного варіанту] грн. за 2-х дорослих\n"
            ...
            f"⚠️ ОБМЕЖЕННЯ: Описуй кожен готель ємно. Твій підсумковий текст має бути не більше за 3000 символів. Використовуй тільки HTML-теги b, i та a для прихованого посилання.\n\n"
        
            f"⚠️ СУВОРЕ ТА КАТЕГОРИЧНЕ ПРАВИЛО ЩОДО СТИЛЮ МІНУСІВ ТА ЗАБОРОНИ НЕВЕРОЯТНОСТІ/НЕВПЕВНЕНОСТІ:\n"
    ...
    if successful_posts_count > 0:
        try:
            ...
            logging.info("✅ Фінальний CTA-пост успішно опубліковано!")
