...
async def generate_and_send_ai_tour_post():
    ...
    # Налаштування категорій з назвами країн ВЕЛИКИМИ літерами та прапорами
    categories = [
        {"name": "ТУРЕЧЧИНА", "flag": "🇹🇷", "prompt_part": "Знайди та вибери РІВНО 5 різних найкращих готелів 4★ та 5★ СУТО в ТУРЕЧЧИНІ."},
        {"name": "ЄГИПЕТ", "flag": "🇪🇬", "prompt_part": "Знайди та вибери РІВНО 5 різних найкращих готелів 4★ та 5★ СУТО в ЄГИПТІ."},
        {"name": "УКРАЇНА", "flag": "🇺🇦", "prompt_part": "Знайди та вибери РІВНО 5 різних найкращих пропозицій або готелів СУТО в УКРАЇНІ."},
        {"name": "ІНШІ КРАЇНИ", "flag": "🌍", "prompt_part": "Знайди та вибери РІВНО 5 різних найкращих готелів 4★ та 5★ з БУДЬ-ЯКИХ ІНШИХ КРАЇН, окрім Туреччини, Єгипту та України."}
    ]
    ...
    for index, cat in enumerate(categories):
        # Робимо паузу 15 секунд між країнами, щоб не блокував ліміт API Gemini
        if index > 0:
            logging.info(f"⏳ Очікуємо 15 секунд перед запитом ШІ для категорії '{cat['name']}'...")
        ...
        prompt = (
            ...
            f"ТВОЄ ГОЛОВНЕ ЗАВДАННЯ: {cat['prompt_part']} Якщо в тексті сайту доступно менше 5 готелів для цієї категорії, то виведи всі, які є.\n\n"
            f"Суворо дотримуйся наступних правил конструювання тексту:\n"
            f"1. НІКОЛИ не згадуй назву сторонніх сайтів (наприклад, тат.уа або інші джерела).\n"
            f"2. Першим рядком поста ОБОВ'ЯЗКОВО має бути виключно заголовок у такому форматі (СУВОРЕ ДОТРИМАННЯ СИМВОЛІВ ТА ПОРЯДКУ):\n"
            f"☀️ <b>ТОП пропозицій: {cat['flag']} {cat['name']}</b>\n"
            f"3. ХУДОЖНІЙ ВСТУП (ОБОВ'ЯЗКОВО): Одразу після заголовка напиши один короткий, емоційний та вступний художній абзац, який яскраво й атмосферно описує переваги відпочинку в цьому напрямку, щоб зацікавити читача.\n"
            f"4. СУВОРЕ ПРАВИЛО ДЛЯ ВСТУПУ: Заборони будь-які технічні чи робочі фрази типу 'Згідно з наявними даними...', 'Ми знайшли пропозицію...', 'На основі наданого тексту...', 'Ось готелі з бази...'. Текст повинен виглядати як особиста рекомендація живого тревел-експерта підписникам, без згадок про пошук чи дані.\n"
            ...
            f"🏨 <b>Назва готелю і зірковість (наприклад: Grand Konakli Resort 5*)</b>\n"
            f"🍽 <b>Харчування:</b> [Вкажи тип харчування з тексту]\n"
            f"📅 <b>Виліт/Дата:</b> [наприклад: 28.05.2026 (7 ночей)]\n"
            f"💰 <b>Ціна:</b> [Вкажи повну вартість, як у тексті]\n"
            f"[Тут напиши короткий художній опис саме цього готелю, його території та умов]\n"
            f"➕ <b>Плюси:</b> [Коротко вкажи переваги готелю]\n"
            f"➖ <b>Мінуси:</b> [Коротко вкажи нюанси або мінуси готелю]\n\n"
            f"6. ОБМЕЖЕННЯ: Описуй кожен готель ємно. Твій згенерований текст разом із заголовком ОБОВ'ЯЗКОВО має бути коротшим за 3400 символів, щоб разом із рекламою поміститися в ліміт Telegram (4096). Використовуй тільки HTML-теги <b> та <i>.\n\n"
