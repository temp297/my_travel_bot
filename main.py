...
async def fetch_tat_ua_data():
    ...
    # --- ЗМІНА 1: Випадковий вибір сторінки, щоб дані щодня змінювалися ---
    # Замість постійної 1-ї сторінки, беремо випадкову сторінку від 1 до 3
    random_page = random.randint(1, 3) 
    logging.info(f"🎲 Сьогодні парсимо сторінку №{random_page} для різноманітності турів.")

    try:
        for country_slug, base_url in country_urls.items():
            logging.info(f"🌍 Збираємо пропозиції для напрямку: {country_slug.upper()}")
            
            # Формуємо URL для випадкової сторінки
            url = base_url if random_page == 1 else f"{base_url}?page={random_page}"
            ...
            try:
                ...
                logging.error(f"❌ Помилка парсингу сторінки {random_page} для {country_slug}: {page_err}")
                continue
...
async def generate_and_send_ai_tour_post():
    ...
    for index, cat in enumerate(categories):
        ...
        prompt = (
            ...
            f"Намагайся НЕ вибирати ті самі готелі, які є очевидними лідерами замовлень, розбав добірку різними варіантами.\n\n"
            ...
            f"1. Спочатку шукай та виводь готелі зірковості 5★ (5*). Якщо в тексті є достатньо готелів 5★ (від 3 до 5 штук), сформуй добірку виключно або переважно з них.\n"
            f"2. Якщо готелів 5★ в тексті немає або їх замало, тільки тоді додавай/переходь до вибору готелів зірковості 4★ (4*).\n"
        ...
        try:
            response = ai_model.generate_content(
                prompt,
                generation_config={"temperature": 0.7} # 0.7 дасть легкий елемент випадковості у виборі та словах
            )
