...
try:
    ...
    ASSISTANT_MINUTE = int(os.getenv("ASSISTANT_MINUTE", "05"))
except ValueError:
...
async def generate_and_send_ai_tour_post():
    ...
    # Налаштування категорій з посиленим алгоритмом пошуку списку
    categories = [
        {"name": "ТУРЕЧЧИНА", "flag": "🇹🇷", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і знайди ВСІ готелі для цієї країни. Твоє завдання — вибрати до 5 НАЙКРАЩИХ РІЗНИХ готелів 4★ та 5★ СУТО в ТУРЕЧЧИНІ. Не зупиняйся на першому знайденому готелі, ти зобов'язаний сформувати список із кількох варіантів (максимум 5). Не виводь лише один готель!"},
        {"name": "ЄГИПЕТ", "flag": "🇪🇬", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і знайди ВСІ готелі для цієї країни. Твоє завдання — вибрати до 5 НАЙКРАЩИХ РІЗНИХ готелів 4★ та 5★ СУТО в ЄГИПТІ. Не зупиняйся на першому знайденому готелі, ти зобов'язаний сформувати список із кількох варіантів (максимум 5). Не виводь лише один готель!"},
        {"name": "УКРАЇНА", "flag": "🇺🇦", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і витягни до 5 РІЗНИХ найкращих пропозицій або готелів СУТО в УКРАЇНІ. Постарайся знайти та вивести кілька варіантів списком!"},
        {"name": "ІНШІ КРАЇНИ", "flag": "🌍", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і витягни до 5 РІЗНИХ найкращих готелів з БУДЬ-ЯКИХ ІНШИХ КРАЇН, окрім Туреччини, Єгипту та України. Сформуй цікавий список пропозицій."}
    ]
    ...
    for index, cat in enumerate(categories):
        ...
        prompt = (
            ...
            f"ТВОЄ ГОЛОВНЕ ЗАВДАННЯ: {cat['prompt_part']} Сформуй список із кількох готелів (5 штук, якщо вони є в тексті сайту). Якщо ти виведеш лише 1 готель за наявності інших — завдання провалено.\n\n"
            ...
            f"4. СУВОРЕ ПРАВИЛО ДЛЯ ВСТУПУ: Заборони будь-які技術ні чи робочі фрази типу 'Згідно з наявними даними...', 'Ми знайшли пропозицію...', 'На основі наданого тексту...'. Текст повинен виглядати як особиста рекомендація живого експерта підписникам.\n"
