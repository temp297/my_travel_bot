...
async def generate_and_send_ai_tour_post():
    ...
    cta_text = (
        f"⚠️ <b>Зверніть увагу: всі ціни вказані за тур є актуальними на сьогодні!</b>\n\n"
    ...
    # ==========================================
    # КРОК 1: ОЧИЩЕННЯ СТАРИХ ПОСТІВ (Виправлено SyntaxError)
    # ==========================================
    ...
    categories = [
        {"name": "ТУРЕЧЧИНА", "flag": "🇹🇷", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і знайди ВСІ готелі для цієї країни. Твоє завдання — вибрати до 5 НАЙКРАЩИХ РІЗНИХ готелів 4★ та 5★ СУТО в ТУРЕЧЧИНІ. Не зупиняйся на першому знайденому готелі, виводь списком декілька варіантів (максимум 5)!"},
        {"name": "ЄГИПЕТ", "flag": "🇪🇬", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і знайди ВСІ готелі для цієї країни. Твоє завдання — вибрати до 5 НАЙКРАЩИХ РІЗНИХ готелів 4★ та 5★ СУТО в ЄГИПТІ. Не зупиняйся на першому знайденому готелі, виводь списком декілька варіантів (максимум 5)!"},
        {"name": "УКРАЇНА", "flag": "🇺🇦", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і витягни до 5 РІЗНИХ найкращих пропозицій або готелів СУТО в УКРАЇНІ. Постарайся знайти кілька варіантів списком!"},
        {"name": "ІНШІ КРАЇНИ", "flag": "🌍", "prompt_part": "Уважно проскануй весь текст джерела від початку до кінця і витягни до 5 РІЗНИХ найкращих готелів з БУДЬ-ЯКИХ ІНШИХ КРАЇН, окрім Туреччини, Єгипту та України."}
    ]
    ...
    for index, cat in enumerate(categories):
        # Пауза 15 секунд між країнами, щоб не блокував ліміт API Gemini
        if index > 0:
        ...
        prompt = (
            ...
            f"ТВОЄ ГОЛОВНЕ ЗАВДАННЯ: {cat['prompt_part']} Ти зобов'язаний сформувати список із кількох готелів (5 штук), якщо вони є в тексті. Не обмежуйся одним варіантом.\n\n"
            f"Суворо дотримуйся наступних правил конструювання тексту:\n"
            f"1. НІКОЛИ не згадуй назву сторонніх сайтів.\n"
            f"2. Першим рядком поста ОБОВ'ЯЗКОВО має бути виключно заголовок суворо у такому форматі:\n"
            f"🔥 <b>Гарячі тури: {cat['flag']} {cat['name']}</b>\n"
            ...
            f"4. СУВОРЕ ПРАВИЛО ДЛЯ ВСТУПУ: Заборони будь-які технічні чи робочі фрази типу 'Згідно з наявними даними...', 'Ми знайшли пропозицію...', 'На основі наданого тексту...'. Текст повинен виглядати як особиста рекомендація живого експерта підписникам.\n"
