...
prompt = (
    ...
    f"📸 КРИТИЧНЕ ЗАВДАННЯ НА ПОЧАТОК ПОСТУ:\n"
    f"Найпершим рядком у твоєму повідомленні (ще ДО художнього вступу) має бути спеціальний маркер для картинки у форматі: "
    f"[ФОТО КРАЇНИ: {cat['name']}]. Це обов'язково! Пост не може починатися з тексту без цього маркеру.\n\n"
    ...
    f"2. Текст ОБОВ'ЯЗКОВО має починатися з маркеру фото країни: [ФОТО КРАЇНИ: {cat['name']}]. Одразу під ним, з нового рядка, пиши ХУДОЖНІЙ ВСТУП: один короткий, емоційний та вступний художній абзац про країну {cat['name']}, який яскраво описує переваги відпочинку в цьому напрямку.\n"
