...
try:
    ...
    raise ValueError(f"{ADMIN_ID}, {REVIEWS_CHAT_ID}, {FEEDBACK_HOUR} та {FEEDBACK_MINUTE} мають бути цілими числами!")
                        ...
                        if response.status_code == 200:
                            ...
                            if "грн" in page_text or "ноч" in page_text:
                                ...
                                for b in raw_hotel_blocks:
                                    ...
                                    if len(cleaned_block) > 100 and ("ноч" in cleaned_block or "★" in cleaned_block):
...
async def generate_and_send_ai_tour_post():
    ...
    for index, cat in enumerate(categories):
        ...
        prompt = (
            ...
            f"2. Тільки якщо готелів 5★ у списку виявиться менше 5 штук, добирай решту з готелів зірковості 4★.\n"
            f"3. Якщо в тексті немає взагалі ні 5★, ні 4★ готелів для цієї країни, тільки в цьому крайньому разі дозволено брати та показувати готелі зірковості 3★.\n"
            ...
            f"Ось текстові дані з готелями СУТО ДЛЯ ЦІЄЇ КРАЇНИ:\n{country_data}"
        )
...
async def process_dest_callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    for reg_data in DIAL_COUNTRIES.values():
        if item_cmd in reg_data.get("items", {}):
...
async def process_meals(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    meal_key = callback_query.data.split("_")[1]
    meal_text = meal_map.get(meal_key, "Будь-яке")
...
async def delayed_feedback_reply(forwarded_msg: types.Message, rating: int):
...
async def main():
    ...
    # 5. Запуск сервера на правильному порті (10000 за замовчуванням для Render)
