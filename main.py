...
ukraine_tz = pytz.timezone('EuropeKyiv')
...
async def show_admin_base(message types.Message, state FSMContext)
    ...
    if not rows
        ...
        for row in rows
            ...
            if row['return_date']
                if row['sent'] == 1
                    feedback_status = fn     └ ✅ Запит на відгук надіслано ({row['return_date']})
                else
                    feedback_status = fn     └ ⏳ Запит на відгук заплановано ({row['return_date']})

            text += f👤 b{name}b — {username} (code{row['user_id']}code){discount}{feedback_status}n
...
async def init_db()
    ...
    async with pool.acquire() as conn
        ...
        try
            ...
            logging.info(fКолонка full_name вже існує або помилка {e})
        ...
        for row in rows
            try
                await bot.send_message(
                    row['user_id'],
                    ✈️ З поверненням! Сподіваємося, Ваш відпочинок був чудовим.nnБудь ласка, оцініть нашу роботу,
...
def rating_kb()
    builder = InlineKeyboardBuilder()
    for i in range(1, 6)
        builder.add(types.InlineKeyboardButton(text=f{i}⭐, callback_data=frate_{i}))
    builder.adjust(5)
    return builder.as_markup()
...
def get_dropdown_countries_kb(open_region_id str = None)
    ...
    if open_region_id is None
        for r_id, r_data in DIAL_COUNTRIES.items()
            builder.row(types.InlineKeyboardButton(text=f📁 {r_data['title']}, callback_data=ftoggle_{r_id}))
        ...
        if region
            ...
            for item_id, item_name in region[items].items()
                builder.row(types.InlineKeyboardButton(text=f📍 {item_name}, callback_data=fselect_country_{item_id}))
...
# ---     country_urls = {
# ---         turkey httpstat.uasearchturkey,
# ---         egypt httpstat.uasearchegypt,
# ---         greece httpstat.uasearchgreece,
# ---         cyprus httpstat.uasearchcyprus,
# ---         ukraine httpstat.uasearchukraine
# ---    }
    country_urls = {
        turkey httpsnavigo.nniif.org.ua,
        egypt httpsnavigo.nniif.org.ua,
        greece httpsnavigo.nniif.org.ua,
        cyprus httpsnavigo.nniif.org.ua,
        ukraine httpsnavigo.nniif.org.ua
    }
    
    # Ротуємо заголовки для повної імітації реального браузера (захист від блокувань)
    headers = {
        User-Agent Mozilla5.0 (Windows NT 10.0; Win64; x64) AppleWebKit537.36 (KHTML, like Gecko) Chrome124.0.0.0 Safari537.36,
        Accept texthtml,applicationxhtml+xml,applicationxml;q=0.9,imageavif,imagewebp,imageapng,;q=0.8,
        Accept-Language uk-UA,uk;q=0.9,en-US;q=0.8,en;q=0.7,
        Referer httpsturne.uauahottours,
                ...
                if country_raw_blocks
                    ...
                    if len(final_chunk)  200
                        cleaned_country_data[country_slug] = (
                            fn=== ПОЧАТОК БЛОКУ КРАЇНИ {country_slug.upper()} ===n
                            f{final_chunk}
                            fn=== КІНЕЦЬ БЛОКУ КРАЇНИ {country_slug.upper()} ===n
                        )
...
async def generate_and_send_ai_tour_post()
    ...
    bot_link1 = httpst.meNavigatorToursBotstart=welcome
    bot_link2 = httpst.meNavigatorToursBotstart=discount
    current_date_str = datetime.now().strftime(%d.%m.%Y)

    cta_text = (
        f⚠️ bЗверніть увагу всі ціни вказані за тур та є актуальними на момент публікації!bnn
        f✈️ Бажаєте забронювати або підібрати інший варіантn
        fНаш електронний помічник допоможе вам швидко сформувати запит, а професійний менеджер особисто опрацює ваші побажання.n
        f👉 a href='{bot_link1}'Залишити запит менеджеруann
        f🎁 bПриємний бонусb кожен наш клієнт може отримати персональну знижку за програмою лояльності!n
        f👉 a href='{bot_link2}'Отримати знижкуann
        f🗣 bСподобалася добіркаbn
        fПоширюйте канал серед знайомих мандрівників — разом шукати вигідні тури цікавіше!
    )
        ...
        if old_rows
            ...
            for row in old_rows
                try
                    ...
                    logging.warning(fНе вдалося видалити старий post {row['message_id']} {del_err})
...
# Базовий шаблон для всіх фото з Unsplash (щоб не дублювати його в коді 75 разів)
    UNSPLASH_BASE_URL = httpsimages.unsplash.com{}fm=jpg&fit=crop&w=1000&h=562&q=75
    ...
    for index, cat in enumerate(categories)
        ...
        prompt = (
            fТи — професійний travel-копірайтер компанії. На основі НАДАНИХ ТЕКСТОВИХ ДАНИХ склади один цікавий, 
            fструктурований і залучаючий пост для Telegram-каналу українською мовою.nn
            fТВОЄ ГОЛОВНЕ ЗАВДАННЯ {cat['prompt_part']}nn

            f📸 КРИТИЧНЕ ЗАВДАННЯ НА ПОЧАТОК ПОСТУ (ПРИХОВАНЕ ФОТО В ЕМОДЗІ)n
            fТи маєш почати художній вступ з емодзі, в який за допомогою HTML-тегу a буде вшито посилання на фотографію країни. 
            fВикористовуй для цього стабільне динамічне посилання a href={country_photo_url}🌍an
            fЦЕ КРИТИЧНО ВАЖЛИВО! Завдяки цьому Telegram автоматично підтягне красиву велику картинку-прев'ю вгору вашого поста, 
            fяка відображає пейзажі країни {cat['name']}, але саме довге посилання не буде видно користувачам у тексті.nn
            
            f🚫  ПРАВИЛО ЛОГІКИ ТРАНСПОРТУ ТА ЕМОДЗІ (ЗАБОРОНА ПЛУТАНИНИ)n
            f1. Автобусні тури маркуються ЛИШЕ символом 🚌 в усьому рядку. Наприклад '🚌 Автобус із Києва' або '🚌 Автобус зі Львова'. Заборонено в цей рядок ставити літак ✈️!n
            f2. Авіатури маркуються ЛИШЕ символом ✈️ в усьому рядку. Наприклад '✈️ Авіа з Кишинева' або '✈️ Авіа з Варшави'. Заборонено в цей рядок ставити автобус 🚌!n
            f3. Ніколи не зліплюй емодзі автобуса з текстом про авіарейси. Кожен тип транспорту має відповідати своему єдиному значку.n
            f4. ЗАБОРОНА ФЕЙКОВИХ ВИЛЬОТІВ З УКРАЇНИ Наразі авіатури з міст України (Київ, Харків, Львів, Одеса тощо) не здійснюються! Якщо тур авіаційний — місто вильоту обов'язково має бути іноземним (Кишинів, Варшава, Жешув, Сучава, Катовіце тощо). З українських міст можливий ТІЛЬКИ автобусний виїзд чи власний транспорт. Не вигадуй вильоти літаків з України!nn

            f❌ КАТЕГОРИЧНА ЗАБОРОНА НА ФРАЗИ 'ЗА ЗАПИТОМ', 'УТОЧНЮЙТЕ' ТА АБСТРАКЦІЇn
            f- Тобі категорично заборонено використовувати у фінальному тексті фрази 'з міст Європи', 'уточнюйте', 'за запитом', 'уточнюйте у менеджера', 'деталі за телефоном', 'кількість ночей за запитом'. Пост повинен містити виключно готову, фіксовану та конкретну інформацію для туриста.n
            f- У полі трансферу та виїзду ОБОВ'ЯЗКОВО має бути чітко вказано конкретне місто вильоту або виїзду. Жодних розмитих формулювань. Якщо це авіатур — проаналізуй контекст і вкажи конкретне закордонне місто, з якого виконується цей рейс (наприклад, Кишинів, Варшава тощо).n
            f- Якщо для якогось готелю в наданому тексті замість конкретної дати, кількості ночей чи типу харчування вказано 'уточнюйте' або 'за запитом' — повністю ІГНОРУЙ такий готель і переходь до аналізу наступних варіантів, де є точні та прописані дані.nn
            
            f⚠️ КРИТИЧНО ВАЖЛИВЕ ПРАВИЛО ПРІОРИТЕТУ ЗІРКОВОСТІ ГОТЕЛІВn
            fТи повинен робити вибірку готелів (до 5 штук) суворо за такому каскадним пріоритетомn
            f1. Наданий тобі список вже відсортований на рівні Python на самому початку йдуть преміум-готелі 5★. Твоє головне завдання — вибрати до 5 найкращих готелів 5★. Сформуй добірку виключно або переважно з них.n
            f2. Тільки якщо готелів 5★ у списку виявиться менше 5 штук, добирай решту з готелів зірковості 4★ (4).n
            f3. Якщо в тексті немає взагалі ні 5★, ні 4★ готелів для цієї країни, тільки в цьому крайньому разі дозволено брати та показувати готелі зірковості 3★ (3).n
            fПорушення цього пріоритету заборонено! Якщо є вища зірковість — нижчу ігноруй.nn
            
            f⚠️ ХУДОЖНІЙ ПЕРЕХІДn
            fУ вступному абзаці обов'язково адаптуй фразу-перехід під фактично обрану зірковість готелів. Наприклад, якщо у списку опинилися лише 5★, напиши 'Ось наша добірка найкращих преміум-готелів 5★...'. Якщо вийшов мікс 5★ та 4★, напиши 'Ось добірка найкращих готелів 4★ та 5★...'.nn
            
            f⚠️ ГОЛОВНИЙ КРИТЕРІЙ ВІДБОРУ — ПРІОРІТЕТ ТРАНСПОРТУ ТА ПОВНОГО ПАКЕТУn
            fПроаналізуй тип транспорту для готелів та відбере варіанти за суворим каскадним пріоритетомn
            f1. ПРІОРІТЕТ №1 — АВІАТУРИ Шукай у тексті готелі, де вказано авіапереліт (літакавіа з Кишинева, Сучави, Жешува, Варшави тощо).n
            f    🔥 КРИТИЧНА ВИМОГА ДЛЯ АВІА Серед усіх знайдених авіатурів ти зобов'язаний ПЕРШОЧЕРГОВО вибирати варіанти, які є ПОВНИМ ПАКЕТОМ (куди одночасно включено Проїзд, Страховка та Трасфер до готелю). Формуй добірку з них. Якщо авіатурів у тексті є хоча б 3-5 штук, повністю ігноруй автобуси та власний транспорт!n
            f2. АВТОБУСНІ ТУРИ Включай автобусні тури у пост ТІЛЬКИ у випадку, якщо в наданих даних взагалі немає авіатурів по цій країні.n
            f    🔥 КРИТИЧНА ВИМОГА ДЛЯ АВТОБУСІВ Аналогічно, серед автобусних турів вибирай насамперед ті, куди включено повний пакет (Проїзд + Страховка + Трасфер).n
            f3. БЕЗ ТРАНСФЕРУ Варіанти 'Власний транспорт  Без трансферу' дозволено брати лише в крайньому разі, якщо немає ні авіа, ні автобусів.n
            fПідсумок Завжди зберігай пріоритет транспорту (Авіа - Автобус), але ВСЕРЕДИНІ обраного транспорту вибирай ТІЛЬКИ ті тури, де чітко включено проїзд, страховку та трансфер!nn
            
            f⚠️ КРИТИЧНО ВАЖЛИВЕ ПРАВИЛО ДЛЯ НАЙНИЖЧОЇ ЦІНИ ТА СИНХРОНІЗАЦІЇn
            f- Коли алгоритм відібрав готелі за пріоритетом трансферу (наприклад, авіа з повним пакетом), вибирай серед них варіанти за ЯКІСТЮ та НАЙВИГІДНІШОЮ ціною для конкретної кількості ночей. n
            f- НІКОЛИ не зліплюй ціну від дешевого автобусного туру з описом авіатуру! Ціна, тип харчування, трансфер та кількість ночей у шаблоні мають бути СУВОРО СИНХРОНІЗОВАНІ між собою для обраного варіанту.n
            f- Якщо для одного готелю є кілька цін (за 4, 5 чи 7 ночей), виводь ту, яка є найпривабливішою, чітко вказавши відповідну їй кількість ночей.n
            f- Уникай 'оверпрайсу' не вибирай готелі з космічними цінами (наприклад, за 250-500 тис. грн), якщо в тексті є чудові варіанти за 60-100 тис. грн. Шукай 'золоту середину' ціни та сервісу.nn
            
            f⚠️ СУВОРЕ ПРАВИЛО ДЛЯ АНАЛІЗУ ТА ФОРМАТУ ДАТ (ТОЧНИЙ ПОВТОР З САЙТУ)n
            f1. Знайди в тексті конкретного туру дату вильотувиїзду (день, месяц, рік), яка прописана поруч із обраною ціною. Заборонено вигадувати дані самостійно.n
            f2. КРИТИЧНО ВАЖЛИВО Переноси дату в шаблон СУВОРО в тому форматі, в якому вона написана в джерелі (на сайті). 
            fЯкщо на сайті вказано цифровий формат (наприклад, '15.06.2026'), пиши '15.06.2026'. 
            fЯкщо на сайті вказано словами (наприклад, '19 червня' або '5 липня'), ти зобов'язаний перенести саме так '19 червня' або '5 липня'. 
            fНічого від себе не змінюй, не трансформуй формати та не додавай зайвих слів чи літер (наприклад, р., рік тощо).nn
            
            f⚠️ Суворо дотримуйся наступних правил конструювання текстаn
            f1. НІКОЛИ не згадуй назву сторонніх сайтів чи парсерів.n     
            f2. Текст ОБОВ'ЯЗКОВО має починатися з ХУДОЖНЬОГО ВСТУПУ, але у найперший емодзі цього вступу ти зобов'язаний «вшити» посилання на фотографію країни за допомогою тегу a. Схема початку тексту a href={country_photo_url}🌍a [Далі йде емоційний художній абзац про країну {cat['name']}, який яскраво описує переваги відпочинку].n
            f3. СУВОРЕ ПРАВИЛО ДЛЯ ВСТУПУn
            f- Заборони будь-які технічні чи робочі фрази типу 'Згідно з наявними даними...', 'Ми знайшли...'. Текст повинен виглядати як рекомендація живого експерта.n
            f4. СУВОРЕ ПРАВИЛО ДЛЯ РОЗДІЛЕННЯ ТУРИВ Відокремлюй картки готелів одну від одної СУВОРО одним порожнім рядком. Категорично ЗАБОРОНЕНО самостійно малювати штучні лінії, ставити знаки мінусів, дефісів чи символи на кшталт '---' між готелями! Тільки порожній рядок.n
            f5. Після художнього вступу виведи список готелів. Для КОЖНОГО готелю суворо використовуй наступний візуальний шаблон (заповнюй дані, зберігаючи емодзі та жирний шрифт, у полі виїзду обов'язково вказуй назву міста)nn
            
            f📍 b{cat['name'].upper()} ([Вкажи регіонкурорт])bn
            f🏨 b[Назва готелю латиницею в оригіналі] [Зірковість, наприклад 4 або 5]bn
            f🚌 bТрансфер та виїздb [Залежно від туру, вкажи чітке місто вильоту чи виїзду! Приклади '✈️ Авіа з Кишинева', '✈️ Авіа з Варшави', '🚌 Автобус із Києва', '🚗 Власний транспорт'. Жодних загальних фраз типу 'міст Європи' чи 'уточнюйте'!]n
            f🍽 bХарчуванняb [Вкажи тип харчування, що відповідає обраній ціні, наприклад 'Все включено (AI)' або 'Без харчування (RO)']n
            f📅 bВилітДатаb [Вкажи точну дату туру з тексту СУВОРО в оригінальному форматі сайту ДД.ММ.РРРР або ДД місяця], [Вкажи кількість ночей саме для цієї ціни]n
            f💰 bЦінаb [Вкажи саме вартість для обраного типу трансферу] грн. за 2-х дорослихn
            fi[Тут напиши короткий художній опис саме цього готелю. Коротко вкажи реальні матеріальні переваги самого готелю інфраструктура, перша лінія, басейни, спа, свіжий ремонт, зелена territory, аквапарк тощо]inn
            
            f⚠️ ДОДАТКОВІ ОБМЕЖЕННЯ ДЛЯ ОФОРМЛЕННЯ ГОТЕЛІВn
            f- СУВОРЕ ПРАВИЛО ДЛЯ ДАТИ Заборонено примусово міняти формат дати. Відображай символ в символ, як у тексті джерела. Без самодіяльності та зайвих знаків.n
            f- СУВОРЕ ПРАВИЛО ДЛЯ НАЗВИ ГОТЕЛЮ Виводь назву готелю в оригіналі так, як вона вказана в тексті джерела (латиницею).nn
            
            f⚠️ ОБМЕЖЕННЯ Описуй каждый готель ємно. Твій підсумковий текст має бути не більше за 3000 символів. Використовуй тільки HTML-теги b, i та a для прихованого посилання.nn
            
            f⚠️ СУВОРЕ ТА КАТЕГОРИЧНЕ ПРАВИЛО ЩОДО СТИЛЮ МІНУСІВ ТА ЗАБОРОНИ НЕВВЕРНЕНОСТІn
            f- КАТЕГОРИЧНО ЗАБОРОНЕНО використовувати конструкції 'Може бути...', 'Здається...', 'Можливо...', 'Не всім сподобається...'. Текст має бути чітким, професійним та констатувати факти.n
            f- Пиши прямо і професійно НЕ 'Може бути завеликим' - А 'Велика територія'; НЕ 'Може бути далеко від моря' - А 'Друга лінія, 400 м до пляжу'; НЕ 'Можливо старі номери' - А 'Класичнийтрадиційний номерний фонд'.n
            f- Також повністю заборонені фрази типу 'Не вказано конкретних матеріальних недоліків в наданому тексті', 'Інформація відсутня'. Якщо даних немає — напиши нейтральний художній нюанс, який підходить усім готелям, але НІКОЛИ не пиши технічні виправдання!.nn
            
            fОсь текстові дані з готелями СУТО ДЛЯ ЦІЄЇ КРАЇНИ {country_data}
        )
        ...
        try
            ...
            header_text = f🧭 bНавігатор дня {cat['name'].upper()} {cat['flag']}  {current_date_str}bnn
...
async def cmd_start(message types.Message, state FSMContext, command CommandObject)
    ...
    if args == discount
        ...
        if existing_discount
            ...
            greeting = fВітаємо, {name}! 🎁 У вас є активна знижка {discount}%.nВикористайте її під час бронювання наступного туру!
        else
            ...
            await pool.execute(
                ...
                , user.id, discount)
            greeting = fВітаємо, {name}! 🎁 Ви активували знижку {discount}%.
    else
        discount_row = await get_user_discount(user.id)
        if discount_row
            greeting = fВітаємо, {name}! 🎁 У вас є активна знижка {discount_row['discount_value']}%.nВикористайте її під час бронювання наступного туру!
        else
            greeting = fВітаємо, {name}! Я допоможу вам підібрати тур.
...
async def cmd_discount(message types.Message, state FSMContext)
    ...
    async with pool.acquire() as conn
        ...
        if row
            ...
            text = f🎁 Вітаємо, {name}, у вас є активна знижка {discount}%nВикористайте її під час бронювання наступного туру!
        else
            chance = random.random()
            if chance  0.70
            ...
            text = fВітаємо, {name}! 🎉 Ви виграли знижку на наступну подорож {discount}%.nВикористайте її під час бронювання наступного туру!
...
async def check_active_discounts(message types.Message, state FSMContext)
    ...
    async with pool.acquire() as conn
        ...
        if not rows
            ...
            return
        text = 🎁 bСписок клієнтів з активними знижкамиbn
        for row in rows
            text += f👤 ID code{row['user_id']}code — {row['discount_value']}%n
...
async def admin_start(message types.Message, state FSMContext)
    ...
    await state.clear()
    msg = await message.answer(🛠 bПанель менеджераbnnВведіть bIDb або bUsernameb клієнта, parse_mode=HTML)
...
async def start_use_discount(message types.Message, state FSMContext)
    ...
    for row in rows
        ...
        kb.row(types.InlineKeyboardButton(
            text=button_text, 
            callback_data=fapply_{row['user_id']})
        )
        
    msg = await message.answer(🎁 bОберіть клієнта для використання знижкиb, reply_markup=kb.as_markup(), parse_mode=HTML)
...
async def process_start_callback(callback_query types.CallbackQuery, state FSMContext)
    ...
    text = (
        🌍 bОберіть напрямок для відпочинку.bnn
...
async def manual_country_selected(callback_query types.CallbackQuery, state FSMContext)
    ...
    msg = await callback_query.message.answer(
        ✍️ Будь ласка, введіть назву країни (та містаготелю за бажанням) текстовим повідомленням,
...
async def process_date_to(callback_query types.CallbackQuery, callback_data SimpleCalendarCallback, state FSMContext)
    ...
    if selected
        ...
        for nights in range(1, 21)
            builder.add(types.InlineKeyboardButton(text=f{nights}, callback_data=fselect_nights_{nights}))
        builder.adjust(5) # Робимо сітку по 5 цифри в ряд
        add_back_button(builder, back_to_date_to)
        
        msg2 = await callback_query.message.answer(🌙 bОберіть кількість ночей відпочинкуb, reply_markup=builder.as_markup(), parse_mode=HTML)
        await save_msg(msg1, state)
        await save_msg(msg2, state)
...
async def process_budget(message types.Message, state FSMContext)
    ...
    msg = await message.answer(
        📞 Будь ласка, натисніть кнопку b«📱 Поділитися контактом»b нижче або введіть свій номернікнейм вручну,
...
async def process_contact(message types.Message, state FSMContext)
    ...
    info_table = (
        f🌍 bНапрямокb {data.get('destination')}n
        f👥 bСкладb {data.get('adults')} дор. + {data.get('children')} діт.n
        f📅 bДати початку туруb {data.get('date_from')} - {data.get('date_to')}n
        f🌙 bНочейb {data.get('nights')}n
        f⭐ bГотельb {data.get('stars')}n
        f🍴 bХарчуванняb {data.get('meals')}n
        f💰 bБюджетb {data.get('budget')} ГРНn
        f🎁 bЗнижкаb {discount_status}n
        f📱 bКонтактb {contact_info}
    )
    
    report = (
        f🔥 bНОВА ЗАЯВКА НА ТУР!bn
        f━━━━━━━━━━━━━━━n
        f{info_table}n
        f━━━━━━━━━━━━━━━n
        f👤 bКлієнтb a href='tguserid={user.id}'{user.full_name}an
        f🆔 bUsernameb @{user.username if user.username else 'немає'}n
        f🆔 bID для відгукуb code{user.id}coden
        f━━━━━━━━━━━━━━━
    )
    ...
    await message.answer(
        f✅ Дякуємо! Заявку успішно відправлено!nМи зв'яжемося з Вами найближчим часом 😊nn
        fbДЕТАЛІ ВАШОЇ ЗАЯВКИbn
        f━━━━━━━━━━━━━━━n
        f{info_table}n
        f━━━━━━━━━━━━━━━, 
        parse_mode=HTML,
...
async def delayed_feedback_reply(forwarded_msg, rating)
    ...
    try
        ...
        logging.error(fError sending delayed reply {e})
...
async def process_admin_search(message types.Message, state FSMContext)
    ...
    if target_id
        ...
        msg = await message.answer(
            f✅ Клієнта знайденоnID code{target_id}codenUser {username}nnТепер оберіть дату повернення, 
...
async def process_admin_date(callback_query types.CallbackQuery, callback_data SimpleCalendarCallback, state FSMContext)
    ...
    if selected
        ...
        report_msg = await callback_query.message.answer(
            f✅ bЗапит на відгук перепризначено!bn
            f━━━━━━━━━━━━━━━n
            f📅 bНова датаb {formatted}n
            f👤 bКлієнтb {username} (code{client_id}code)n
            fi(Старі запити для цього клієнта скасовано)in
            f━━━━━━━━━━━━━━━,
            parse_mode=HTML
        )
